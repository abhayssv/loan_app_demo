...
var removeVideoInfo = function (images) { 
  if (images) {
    var { selfie_video } = images; 
      selfie_video.map(element =>
        removeVideo(element)
      );
  }
} 
...
function removeVideo(video) {
  if (video != undefined && video != "") {
    var imagePath = path.resolve(generalConfig.filesPath.userVideo + video); 
    fs.stat(imagePath, function (err, stat) {
      if (err == null) {
        fs.unlink(imagePath, function (err) {
          if (err) throw err;
...
var getVideoProof = function (videoInfo, callback) {
  if (videoInfo) {
    ...
    if (videoInfo) { 
      try {
        ...
        const {selfie_video } = videoInfo
        var selfie = selfie_video.map(element =>
          generalConfig.serverUrl + '/' + generalConfig.filesPath.userVideo + element
        )
        videoInfo['selfie_video'] = selfie;
      } catch (e) {
        console.log(e);
      }
    } else {
      console.log("Video Proof Not found issue in video mapping")
    }
    callback();
  }
};
...
module.exports = {
  ...
  removeVideoInfo: removeVideoInfo,
  getVideoProof: getVideoProof
}
