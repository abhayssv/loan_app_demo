...
module.exports = function () {
  ...
  return {
    ...
    validateEmailPassword(email, password, res) {
      // Login
      db.models.user.findOne({ where: { email: email, active: 1 } })
        .then(function (user) { 
          if (user) {
    ...
    validateAppEmail(email, mobile_no, res) {
      // Login
      db.models.appUser.findOne({ where: { email: email, mobile_no: mobile_no ,status: 1 } })
        .then(function (user) {
          if (!user) return next(new AppError('Oops! User credentials are invalid.', 404))
          res({ message: 'Login successful.', error: false, data: user });
            }).catch(function (err) {
          res({ message: 'Oops! Something went wrong.', error: true, data: null });
        });
