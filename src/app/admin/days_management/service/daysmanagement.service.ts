...
export class DaysManagementService {
  private requestUrl = environment.requestUrl + '/api/user_time'; // URL to Web API
  private DaysManagement: any;
  httpOptions = {
    headers: new HttpHeaders({'content-Type': 'application/json;charset=UTF-8'})
  };
  constructor(private http:HttpClient, private alertService: AlertService) { }

  getDaysManagements():Observable<User>{
    return this.http.get(this.requestUrl + '/admin/all').pipe(
      tap((res: any)=>{  
        this.handleResponse(res);
      }),
      catchError(this.handleError<any>('DaysManagement'))
    );
  } 
